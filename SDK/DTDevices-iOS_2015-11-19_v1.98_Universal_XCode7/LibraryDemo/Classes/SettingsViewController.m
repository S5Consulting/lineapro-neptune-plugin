...
static NSString *mic_gains[]={
    @"Infinea X Mic gain 5",
    @"Infinea X Mic gain 10",
    @"Infinea X Mic gain 15",
    @"Infinea X Mic gain 20",
};
...
-(NSString *)getFirmwareFileName
{
    if([dtdev getSupportedFeature:FEAT_PIN_ENTRY error:nil])
    {
        ...
        //NSString *file=[path stringByAppendingPathComponent:@"BLUPAD500_FW_2.4.44.0_S1.0.0.0.BIN"];
        NSString *file=[path stringByAppendingPathComponent:@"PINPAD_AP_2.4.37.0_S1.0.0.0.BIN"];
        return file;
    }
...
-(BOOL)getFilesWithPrefix:(NSString *)prefix extension:(NSString *)extension
{
    NSString *path=[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"Firmware"];
    ...
    {
        NSString *file=[[files objectAtIndex:i] lastPathComponent];
        if([[file lowercaseString] hasPrefix:prefix] && [[file lowercaseString] hasSuffix:extension])
        {
...
-(void)checkForOpticonFirmwareUpdate;
{
    ...
    if([self getFilesWithPrefix:@"opticon" extension:@".bin"])
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Firmware Update",nil)
...
-(void)checkForNewlandFirmwareUpdate;
{
    ...
    if(r)
    {
        ...
        if(r)
        {
            NSString *engine=[[NSString alloc] initWithData:r encoding:NSASCIIStringEncoding];
    ...
    if([self getFilesWithPrefix:@"newland_" extension:@".bin"])
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Firmware Update",nil)
...
-(void)checkForIntermecFirmwareUpdate;
{
    ...
    if([self getFilesWithPrefix:@"intermec_" extension:@".bin"])
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Firmware Update",nil)
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	...
	switch (section)
	{
        ...
        case SEC_MIC:
            if(dtdev.connstate==CONN_CONNECTED && [dtdev getSupportedFeature:FEAT_LEDS error:nil]!=FEAT_UNSUPPORTED)
                nRows=sizeof(mic_gains)/sizeof(mic_gains[0]);
            break;
...
//            }
        ...
        case SEC_MIC:
        {
            int gains[]={5,10,15,20};
            
            if(![dtdev btSetMicGain:gains[indexPath.row] error:&error])
            {
                ERRMSG(NSLocalizedString(@"Command failed",nil));
            }
            [tableView reloadData];
        ...
        case SEC_TRANSARMOR:
            ...
            if(indexPath.row==1) {//set Encrypted T1 only
            ...
            if(indexPath.row==2) {//set Encrypted T2 only
            ...
            if(indexPath.row==3) {//set Encrypted T3 only
            ...
            if(indexPath.row==4) {//set Encrypted PAN only
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	switch ([indexPath indexAtPosition:0])
	{
        ...
        case SEC_MIC:
            cell.accessoryType=UITableViewCellAccessoryNone;
            [cell.textLabel setText:NSLocalizedString(mic_gains[indexPath.row],nil)];
            break;
...
{
    [self getFilesWithPrefix:@"intermec_" extension:@".bin"];
