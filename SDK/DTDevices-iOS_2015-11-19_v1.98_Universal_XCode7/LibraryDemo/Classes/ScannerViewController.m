...
-(NSString *)buildInfoString:(bool)force
{
    ...
    if(dtdev.connstate==CONN_CONNECTED)
    {
        ...
        NSTimeInterval time;
        if([dtdev getTimeRemainingToPowerOff:&time error:nil])
            initString = [initString stringByAppendingFormat:@"\nAutoff time: %.02f",time];
        else
            initString = [initString stringByAppendingFormat:@"\nAutoff time: NA"];
        
        if(!emsrString && [dtdev getSupportedFeature:FEAT_MSR error:nil]&(MSR_ENCRYPTED|MSR_VOLTAGE))
        {
...
//        [dtdev uiControlLEDsWithBitMask:0xFFFF error:nil];
//        [dtdev rfInit:0xFFFF error:nil];
    }else
    {
...
-(void)magneticCardReadFailed:(int)source
{
    ...
    [status setString:@"Card read failed"];
}
