...
static uint8_t dataKey[16]={0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31,0x31};
...
-(IBAction)onSetKeysHMK:(id)sender;
{
    ...
    [self changeKeyTR31WithHMK:"B1" mode:'X' key:[NSData dataWithBytes:ipek length:sizeof(ipek)] keyAddon:[NSData dataWithBytes:DUKPT_KSN1 length:sizeof(DUKPT_KSN1)] keyID:2 keyVersion:3];
...
-(NSString *)ppadGetPINBlockUsingDUKPT:(NSError **)error
{
    ...
    NSData *pinData=[dtdev ppadGetPINBlockUsingDUKPT:2 keyVariant:nil pinFormat:PIN_FORMAT_ISO1 error:error];
...
-(IBAction)onManualCardEntry:(id)sender
{
    ...
    //set to disable CVV on manual card entry
    [dtdev emv2SetManualEntryOptionsEnableCVV:true enableExpiration:false error:nil];
