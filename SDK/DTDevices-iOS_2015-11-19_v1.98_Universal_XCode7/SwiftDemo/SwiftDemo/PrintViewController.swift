...
class PrintViewController: UIViewController, DTDeviceDelegate {
    ...
    func paperStatus(present: Bool) {
        lbPaperStatus?.hidden = present
    }
    
    @IBAction func onFontsDemo()
    {
        do {
            try lib.prnPrintText("{=C}FONT SIZES" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{=F0}Font 9x16\n{+DW}Double width\n{-DW}{+DH}Double height\n{+DW}{+DH}DW & DH" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{=F1}Font 12x24\n{+DW}Double width\n{-DW}{+DH}Double height\n{+DW}{+DH}DW & DH" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{=C}FONT STYLES\n{=L}Normal\n{+B}Bold\n{+I}Bold Italic{-I}{-B}\n{+U}Underlined{-U}\n{+V}Inversed{-V}\n" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{=C}FONT ROTATION\n{=L}{=R1}Rotated 90 degrees\n{=R2}Rotated 180 degrees\n" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{+W}{=F0}This function demonstrates the use of the built-in word-wrapping capability" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{+W}{=F1}This function demonstrates the use of the built-in word-wrapping capability" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{+W}{=F0}{=J}This function demonstrates the use of the built-in word-wrapping capability and the use of justify" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{+W}{=F1}{=J}This function demonstrates the use of the built-in word-wrapping capability and the use of justify" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintText("{+W}{=L}Left {=R}and right aligned" , usingEncoding:NSWindowsCP1252StringEncoding)
    ...
    @IBAction func onBarcodesDemo()
    {
        do {
            ...
            try lib.prnPrintText("UPC-A" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.UPCA, barcode:"12345678901".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nUPC-E" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.UPCE, barcode:"012340000040".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nJAN13(EAN)" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.EAN13, barcode:"123456789012".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nJAN8(EAN)" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.EAN8, barcode:"96385074".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nCODE 39" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.CODE39, barcode:"1A1234567".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nITF" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.ITF, barcode:"123456789012".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nCODABAR (NW-7)" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.CODABAR, barcode:"A12356789A".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nCODE 93" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.CODE93, barcode:"AABCD12345".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nCODE 128" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcode(BAR_PRN.CODE128, barcode:"BABCD12345".dataUsingEncoding(NSASCIIStringEncoding))
            try lib.prnPrintText("\nPDF-417" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcodePDF417("Hey try to read this :)".dataUsingEncoding(NSASCIIStringEncoding), truncated:false, autoEncoding:true, eccl:PDF417_ECCL._AUTO, size:PDF417_SIZE.W2_H15)
            try lib.prnPrintText("\nQRCODE" , usingEncoding:NSWindowsCP1252StringEncoding)
            try lib.prnPrintBarcodeQRCode("Hey try to read this :)".dataUsingEncoding(NSASCIIStringEncoding), eccl:QRCODE_ECCL._7, size:QRCODE_SIZE._6)
    ...
    @IBAction func onGraphicsDemo()
    {
        do {
            if(lib.pageIsSupported())
            {
                ...
                try lib.prnPrintImage(img, align:ALIGN_CENTER)
                try lib.pagePrint()
                try lib.pageEnd()
            }else
            {
                try lib.prnPrintImage(UIImage(named: "taz.png"), align:ALIGN_CENTER)
            }
    ...
    func print2InchTicket()
    {
        ...
        do {
            ...
            try lib.pageRectangleFrame(lineSize, top:lineSize, width:width-2*lineSize, height:height-2*lineSize, framewidth:lineSize, color:UIColor.blackColor())
            
            let img=UIImage(named: "printer1.png")
            try lib.pageSetWorkingArea(15, top:(height-Int32(img!.size.height))/2, width:-1, height:-1)
            try lib.prnPrintImage(img, align:ALIGN_LEFT)
            ...
            try lib.prnPrintBarcode(BAR_PRN.CODE128AUTO, barcode:"BABCD12345".dataUsingEncoding(NSASCIIStringEncoding))
    ...
    func print3InchTicket()
    {
        ...
        do {
            ...
            try lib.pageFillRectangle(910, top:0, width:2, height:height, color:UIColor.blackColor())
    ...
    func print4InchTicket()
    {
        ...
        do {
            ...
            try lib.pageStart()
            try lib.pageSetWorkingArea(0, top:0, width:width, heigth:height, orientation:PAGE_VERTICAL_TOPRIGHT)
            try lib.pageRectangleFrame(hoffset+lineSize, top:lineSize, width:width-2*lineSize, height:height-2*lineSize, framewidth:lineSize, color:UIColor.blackColor())
            try lib.pageFillRectangle(hoffset+207, top:20, width:lineSize, height:200, color:UIColor.blackColor())
            try lib.pageFillRectangle(hoffset+20, top:220, width:width-2*20, height:lineSize, color:UIColor.blackColor())
            try lib.pageFillRectangle(hoffset+20, top:295, width:width-2*20, height:lineSize, color:UIColor.blackColor())
            
            let p=UIImage(named: "P.png")!
            try lib.pageSetWorkingArea(hoffset+56, top:46, width:-1, height:Int32(p.size.height))
            try lib.prnPrintImage(p, align:ALIGN_LEFT)
            
            try lib.pageSetWorkingArea(hoffset+254, top:35, width:-1, height:192-35)
            try lib.prnPrintText("US POSTAGE\nmPOS\n")
            try lib.prnPrintBarcode(BAR_PRN.PDF417, barcode:"Test barcode".dataUsingEncoding(NSASCIIStringEncoding))
            ...
            try lib.prnPrintBarcode(BAR_PRN.CODE128AUTO, barcode:"420221529405511201080106322512".dataUsingEncoding(NSASCIIStringEncoding))
