...
class CryptoEMSR: CryptoTemplate {
    ...
    func update() throws {
        ...
        //keys
        for key in (emsrKeysInfo.keys as! [EMSRKey]) {
            if key.keyVersion != 0 {
    ...
    func AESEncryptWithKey(data: NSData, key:NSData) -> NSData? {
        return AESOperation(data, operation:CCAlgorithm(kCCEncrypt), key:key);
    }
    
    func AESDecryptWithKey(data: NSData, key:NSData) -> NSData? {
        return AESOperation(data, operation:CCAlgorithm(kCCDecrypt), key:key);
    }
    ...
    func emsrGenerateKeyData(keyID: Int32, keyVersion: Int, keyData: [UInt8], kekData: [UInt8]?) -> [UInt8] {
        ...
        var hashed: [UInt8] = []
        hashed.appendContentsOf(data)
        hashed.appendContentsOf(keyData) //key data
        let hash = SHA256(NSData(bytes: hashed, length: hashed.count))
        
        hashed.appendContentsOf(hash.getBytes())
        
        //encrypt the data if using the encryption key
        if kekData != nil {
            let toEncrypt=NSData(bytes:&hashed[keyStart], length: hashed.count-keyStart)
            let encrypted=AESEncryptWithKey(toEncrypt, key: kekData!.getNSData())
            
            //store the encryptd data back into the packet
            data.appendContentsOf(encrypted!.getBytes())
        }else {
    ...
    func loadKeyID(keyID: Int32, keyData:[UInt8], keyVersion:Int, kekData:[UInt8]?) -> Bool {
        ...
        let generatedKeyData = emsrGenerateKeyData(keyID, keyVersion: keyVersion, keyData: keyData, kekData: kekData)
        //get the key name, for display purposers
        let keyName = EMSRKeysInfo.keyNameByID(keyID)
        
        do {
            //try to load the key in the slot
            try lib.emsrLoadKey(generatedKeyData.getNSData())
            Utils.showMessage("Success!", message: "Key \(keyName) loaded successfully!")
            return true
        } catch let error as NSError {
            Utils.showError("Loading \(keyName) failed!", error: error)
        }
        return false;
    }
            ...
            if row == 1 {
                ...
                if result {
                    ...
                    do {
                        ...
                        if kekVer <= 0 {
                            result = loadKeyID(KEY_EH_AES256_LOADING, keyData: Constants.keyAES256KEK, keyVersion: 2, kekData: nil)
                        }
                    }
                }
                if result {
                    //load aes256 data key
                    result = loadKeyID(KEY_EH_AES256_ENCRYPTION1, keyData: Constants.keyAES256Data, keyVersion: 2, kekData: Constants.keyAES256KEK)
                }
            }
        } catch let error as NSError {
